...
# Создайте запрос для выбора имен менеджеров и их сотрудников: stmt
stmt = select(
    [managers.columns.name.label('manager'),
     employees.columns.name.label('employee')]
)

# Сопоставление идентификаторов менеджеров с идентификаторами сотрудников mgr: stmt_matched
stmt_matched = stmt.where(managers.columns.id == employees.columns.mgr)

# Закажите выписку по имени менеджера: stm_ordered
