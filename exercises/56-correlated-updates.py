...
# Решение -----------------------------------------
# Создайте инструкцию для выбора name из city_fact: circuit_stmt
circuit_stmt = select([city_fact.columns.name])

# Добавьте предложение where, чтобы сопоставить circuit_court с flat_census circuit_code: circuit_stmt
circuit_stmt = circuit_stmt.where(
    city_fact.columns.circuit_court == flat_census.columns.circuit_code)

# Создайте инструкцию update, чтобы задать name circuit_stmt_where: update_stmt
