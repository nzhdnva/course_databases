...
# Напишите функцию для обновления type на "city": update_stmt
update_stmt = update(city_fact).values(type = "city")

# Добавьте метод where(), чтобы ограничить его записями для города Питонбург
update_stmt = update_stmt.where(city_fact.columns.name == 'Питонбург')

# Выполнить update: update_results
